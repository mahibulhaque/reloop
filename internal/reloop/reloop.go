package reloop

import "time"

type (
	// JobID is a 5-character alphanumeric handle assigned at insert time.
	JobID string

	// JobKind identifies whether a job is recurring or one-shot.
	JobKind string

	// JobStatus identifies whether a job can still fire.
	JobStatus string

	// RunStatus identifies the outcome of a recorded run.
	RunStatus string
)

const (
	// KindCron is a recurring cron-style job.
	KindCron JobKind = "cron"

	// KindOneshot is a job scheduled for one wall-clock instant.
	//
	// It runs at most once: the claim clears NextFireAt and opens the
	// RunRunning record in one transaction, so a crash mid-run cannot
	// double-fire it. A record stranded by such a crash is resolved at
	// the next daemon start: closed as RunInterrupted, job marked done.
	KindOneshot JobKind = "oneshot"
)

const (
	// StatusEnabled means the scheduler may fire the job.
	StatusEnabled JobStatus = "enabled"

	// StatusDisabled means the scheduler must skip the job.
	StatusDisabled JobStatus = "disabled"

	// StatusDone means a one-shot job has already fired.
	StatusDone JobStatus = "done"
)

const (
	// RunOK means a command exited successfully.
	RunOK RunStatus = "ok"

	// RunFail means a command failed to start or exited non-zero.
	RunFail RunStatus = "fail"

	// RunRunning means the command is executing right now. A record
	// still running with no live daemon marks a run interrupted by a
	// crash. The next daemon start resolves it to RunInterrupted.
	RunRunning RunStatus = "running"

	// RunSkippedOverlap means a recurring fire was skipped because the prior run was active.
	RunSkippedOverlap RunStatus = "skipped_overlap"

	// RunInterrupted means the daemon exited before the run finished,
	// so the real outcome is unknown. Distinct from RunFail because
	// the command may have succeeded.
	RunInterrupted RunStatus = "interrupted"
)

// Job is a stored, addressable schedule entry.
type Job struct {
	ID      JobID    `json:"id"`
	Kind    JobKind  `json:"kind"`
	Name    string   `json:"name"`
	Command []string `json:"command"`
	// Env is captured at `reloop add` time.
	//
	// It is kept so daemon-run jobs see the user's PATH.
	// It is omitted from JSON because envs often contain secrets.
	Env        []string  `json:"-"`
	Cron       string    `json:"cron,omitempty"`   // non-empty when Kind == KindCron
	FireAt     time.Time `json:"fire_at,omitzero"` // non-zero when Kind == KindOneshot
	Status     JobStatus `json:"status"`
	LastRunAt  time.Time `json:"last_run_at,omitzero"`
	LastStatus RunStatus `json:"last_status,omitempty"`
	// NextFireAt is the next wall-clock fire time.
	//
	// The store maintains it on insert, enable, and claim.
	// Zero means the job never fires again.
	// The scheduler sleeps on this value and does not recompute it.
	NextFireAt time.Time `json:"next_fire_at,omitzero"`
	CreatedAt  time.Time `json:"created_at"`
	UpdatedAt  time.Time `json:"updated_at"`
}

// JobSpec is the user-supplied subset used to create a job.
//
// The store fills in ID and timestamps.
// Exactly one of Cron or FireAt must be set.
// A non-empty Cron creates a cron job.
// Otherwise the spec creates a one-shot job.
type JobSpec struct {
	Name    string
	Command []string
	Env     []string
	Cron    string
	FireAt  time.Time
}

// Run is a recorded execution of a job.
//
// The captured stdout and stderr blob is kept separately.
// Fetch it via the store's OpenRunLog.
type Run struct {
	ID         int64     `json:"id"`
	JobID      JobID     `json:"job_id"`
	StartedAt  time.Time `json:"started_at"`
	FinishedAt time.Time `json:"finished_at,omitzero"`
	ExitCode   int       `json:"exit_code"`
	Status     RunStatus `json:"status"`
}

// DaemonStatus reports the supervisor state observed at probe time.
type DaemonStatus struct {
	Running    bool      `json:"running"`
	PID        int       `json:"pid,omitempty"`
	StartedAt  time.Time `json:"started_at,omitzero"`
	Supervised bool      `json:"supervised"` // true when a supervisor unit is installed
}

// JobCounts is the per-kind/per-state aggregate used by `reloop status`.
// Every job lands in exactly one state bucket, so the buckets sum to
// the kind totals.
type JobCounts struct {
	Total int `json:"total"`
	Cron  int `json:"cron"`
	// CronDisabled is the subset of Cron that will not fire until
	// re-enabled.
	CronDisabled   int `json:"cron_disabled"`
	OneshotPending int `json:"oneshot_pending"`
	OneshotDone    int `json:"oneshot_done"`
	// OneshotDisabled counts one-shots disabled before firing. A
	// one-shot disabled mid-run counts as in flight instead: disabling
	// does not stop a running command.
	OneshotDisabled int `json:"oneshot_disabled"`
	// OneshotInFlight counts one-shots claimed but not yet finished:
	// a zero NextFireAt without StatusDone. While the daemon runs this
	// means the job is executing right now. With the daemon down it
	// means the daemon exited mid-run. The next daemon start marks such
	// jobs done and records the interruption. See KindOneshot.
	OneshotInFlight int `json:"oneshot_in_flight"`
}

// Status aggregates daemon + storage info for `reloop status`.
type Status struct {
	Daemon  DaemonStatus `json:"daemon"`
	DataDir string       `json:"data_dir"`
	DBPath  string       `json:"db_path"`
	Jobs    JobCounts    `json:"jobs"`
}
