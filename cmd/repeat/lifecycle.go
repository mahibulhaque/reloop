...
func jobActionRunE(verb string, act func(*service, context.Context, repeat.Job) error) func(*cobra.Command, []string) error {
...
func newDisableCmd() *cobra.Command {
	return &cobra.Command{
		...
		Long:    "Mark a job disabled so the scheduler stops firing it. The job stays in the store and can be re-enabled later with 'repeat enable'. No-op if already disabled.",
		Example: "  repeat disable backup\n  repeat disable 7K3px",
		Args:    jobArg,
...
// "bad filter".
func parseKindFlag(s string) (repeat.JobKind, error) {
	if s == "" {
		return "", nil
	}
	k := repeat.JobKind(s)
	if k != repeat.KindCron && k != repeat.KindOneshot {
...
// from "bad filter".
func parseStatusFlag(s string) (repeat.JobStatus, error) {
	if s == "" {
		return "", nil
	}
	st := repeat.JobStatus(s)
	switch st {
	case repeat.StatusEnabled, repeat.StatusDisabled, repeat.StatusDone:
		return st, nil
	}
